...
from resume.db.vector_store_manager import VectorStoreManager
...
class ResumeChatbot:
    def __init__(self, gcs_bucket: str, gcs_projects_path: str, gcs_qna_path: str, gcs_introduce_path: str, use_gcs = True, cache_file: str = "answer_cache.json"):
        ...
        self.retriever = Retriever(self.client, db.vectordb)
        self.persona = Persona(self.client, self.history_store)
    ...
    async def chat(self, message: str, history: list, session_id: str):
        ...
        # 1) 질문 분류
        category = await self.classifier.classify_question(message)

        # 2) 관련 컨텍스트 검색
        context = await self.retriever.retrieve_context(message, category)
        ...
        draft_answer = await self.persona.persona_answer(message, category, context)
        ...
        # 5) 대화 기록 저장
        self.history_store.save(session_id, message, final_answer)
